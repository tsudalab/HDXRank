...
def main():
    ...
    parser.add_argument('-name', type=str, default='HDXRank_GN56_cls1', help='model prefix')
    args = parser.parse_args()

    seeds = [42, 43, 44, 45, 46] # random seeds for repeat training
    ...
    for i in range(args.repeat):
        ...
        train_apo, val_apo = train_test_split(apo_input, test_size=args.train_val_split, random_state=seeds[i])
        train_complex, val_complex = train_test_split(complex_input, test_size=args.train_val_split, random_state=seeds[i])
        ...
        train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True) # use all data to train the final model
        ...
        rmse_train_list, rp_train = train_model(model, optimizer, scheduler, loss_fn, train_loader, val_loader, device, args.epoch, args.save, i)

        model_name = f'{args.name}_epoch{args.epoch}_v{i}.pth'
