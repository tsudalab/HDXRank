...
from hdxrank.HDXRank_utils import load_embedding
...
class pepGraph(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(self.tasks["GeneralParameters"]["HDX_File"], str):
            hdx_filename = self.tasks["GeneralParameters"]["HDX_File"]
        elif isinstance(self.tasks["GeneralParameters"]["HDX_File"], list):
            hdx_filename = self.tasks["GeneralParameters"]["HDX_File"][index]
        target_HDX_fpath = os.path.join(self.hdx_dir, f'{hdx_filename}.xlsx')

        is_complex = self.tasks['TaskParameters']['is_complex'][index]
        logging.info(f'Processing task: {hdx_filename} {pdb_fname}')
        apo_states = self.tasks['apo_states'][index] if self.params['mode'] == 'train' else self.tasks['apo_states']
        ...
        load_pep(target_HDX_fpath, apo_states, 
