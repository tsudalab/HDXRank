...
from hdxrank.BioWrappers import *
from hdxrank.GVP_strucFeats import *
...
def config_process(config_file):
    ...
    if general_settings.get("Mode", "single") == "single":
        apo_states = []
        complex_states = []
    ...
    config.setdefault("TaskParameters", {}).update(task_settings)
    config.setdefault("ScorerParameters", {}).update(scorer_settings)
    config.setdefault("GeneralParameters", {}).update(general_settings)
    return config

# TODO: modify parse_xlsx_task to return a list of tasks
# batch mode requires under same structure, apo and complex state exists
# train mode dont require 
def parse_xlsx_task(tasks):
    ...
    tasks['structure_list'] = list(df['structure_file'].astype(str).unique())
    protein_chain_hhms = {}
    apo_states = []
    complex_states = []
    hdx_filename = []
    ...
    if tasks['GeneralParameters']['Mode'] == 'train':
        for temp_apo in tasks['structure_list']:
            ...
            hdx_filename.append(temp_df['database_id'].iloc[0])
            is_complex.append(temp_df['complex_state'].iloc[0])

            protein = temp_df['protein'].astype(str).to_list()
            state = temp_df['state'].astype(str).to_list()
            chain_id = temp_df['chain_identifier'].astype(str).str.upper().to_list()
            correction = temp_df['correction_value'].astype(int).to_list()
            hhm_prefix = temp_df['structure_file'].astype(str).str.upper().to_list()

            # training don't need to match apo and complex states
            apo_states.append([(p,s,c,h,i,h) for p,s,c,i,h in zip(protein, state, correction, chain_id, hhm_prefix)])
            protein_chain_hhms.update({i: f"{h}_{i}" for i,h in zip(chain_id, hhm_prefix)})
        tasks.setdefault("TaskParameters", {}).update({"Switch": True})
        tasks.setdefault("PredictionParameters", {}).update({"Switch": False})
        tasks.setdefault("ScorerParameters", {}).update({"Switch": False})
    else:
        raise ValueError(f"Invalid mode: {tasks['GeneralParameters']['Mode']}")

    tasks['complex_states'] = complex_states
    tasks['apo_states'] = apo_states
    tasks['protein_chain_hhms'] = protein_chain_hhms
    tasks['TaskParameters']['is_complex'] = is_complex
    tasks['GeneralParameters']['HDX_File'] = hdx_filename
    return tasks
...
def parse_task(input_file):
    ...
    if "Mode" in tasks["GeneralParameters"]:
        ...
        if mode.lower() in ['batch', 'train']:
            tasks = parse_xlsx_task(tasks)
            logging.info(f"Parsed {len(tasks['structure_list'])} structures to create embeddings")
