...
import warnings
from hdxrank.BioWrappers import get_bio_model
from hdxrank.HDXRank_utils import load_protein, load_nucleic_acid, load_sm, RawInputData, parse_task
...
def batch_embedding(tasks):
    ...
    df = df[df['structure_file'].isin(tasks['structure_list'])]
    os.makedirs(save_dir, exist_ok=True)

    for _, row in df.iterrows():
        file_string = str(row['structure_file']).upper().split('.')[0]
        pdb_fname = file_string.split(':')[0]
        protein_chain = row['protein_chain'].split(',')
        pep_range = row['pepRange']
        ...
        if isinstance(pep_range, str) and ':' in pep_range:
            ...
            tasks["TaskParameters"]["PepRange"] = pep_range_dict
        else:
            tasks["TaskParameters"]["PepRange"] = {}

        protein_chain_hhms = {chain: f"{pdb_fname}_{chain}" for chain in protein_chain}
        embed_protein(structure_dir, hhm_dir, save_dir, pdb_fname, protein_chain_hhms, NA_chain=[], SM_chain=[], pepRange=tasks['TaskParameters'].get('PepRange', {}))
