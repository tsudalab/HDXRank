...
import pandas as pd
import numpy as np
from hdxrank.GearNet_revise import GearNet
...
def load_data(pepGraph_dir, batch_size=64):
    #graph_list = [graph.strip().split('.')[0].upper() for graph in keys[3]]
    batch_list = []
    input_data = []
    for graph in tqdm(os.listdir(pepGraph_dir), desc='Loading graphs', ncols=0):
        load_fpath = os.path.join(pepGraph_dir, f'{graph}')
        if not os.path.exists(load_fpath):
            logging.info(f'Missing file: {load_fpath}')
            continue
        graph_data = torch.load(load_fpath)
        input_data.extend(graph_data)
        batch_list.extend([graph.split('.')[0]] * len(graph_data))
    logging.info(f'total data:{len(input_data)}')
    return data.DataLoader(input_data, batch_size=batch_size, shuffle=False), np.array(batch_list)

def HDXRank_prediction(tasks):
    ...
    input_dim = tasks["TrainParameters"]["InputDim"]
    hidden_dims = tasks["TrainParameters"]["HiddenDims"]

    Pred_dataloader, batch_list = load_data(pepGraph_dir, batch_size=tasks["PredictionParameters"]["BatchSize"])

    # model loading
    for i, model_path in enumerate(model_list):
        model = GearNet(input_dim = input_dim, hidden_dims = hidden_dims,
        ...
        chain_list = np.array([chain_letters[int(c)] for c in chain_list])
